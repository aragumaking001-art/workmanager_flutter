...
class SwapView(ft.Column):
    def __init__(self):
        ...
        self.total_display = ft.Text(value="", size=36, weight="bold", color=self.theme_color)
        ...
        self.side_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Divider(color="#33363F"),
                ft.Column([
                    ft.Text(f"🏁 本日交換目標", size=14, color=ft.Colors.GREY_400, weight="bold"),
                ...
                ft.Divider(color="#33363F"),
                ft.Row([ft.Text("🛠️", size=18), ft.Text(" 本日交換実績", size=16, weight="bold", color=self.theme_color)]),
        ...
        self.controls = [
            ft.Stack([
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Row([ft.Icon(ft.Icons.SETTINGS_OUTLINED, color=self.theme_color, size=24), ft.Text("筐体交換 ランキング", size=22, weight="bold"), self.weekly_period_label, ft.Container(expand=True), self.mode_selector], vertical_alignment="center"),
                            ft.Row([
    ...
    def load_model_info(self):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                info_dict[key] = {"abbr": str(row['メーカー略称']).strip()}
            return info_dict
        except: return {}
        ...
        if not df_rank.empty:
            ...
            for i in range(3):
                ...
                for idx, (s_id, points) in enumerate(subset.items()):
                    ...
                    font_size = 18 if len(name) <= 6 else 14
    ...
    def open_target_dialog(self, e):
        ...
        self.target_dialog = ft.AlertDialog(title=ft.Text("目標台数の変更"), content=ft.Column([ft.Text("本日の筐体交換目標を入力してください。"), self.target_input], height=120, tight=True), actions=[ft.TextButton("キャンセル", on_click=self.close_target_dialog), ft.ElevatedButton("確定", on_click=self.change_target_count, bgcolor="#00ccff", color="black")])
