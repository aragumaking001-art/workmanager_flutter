...
class AirView(ft.Column):
    def __init__(self):
        ...
        self.total_display = ft.Text(value="", size=36, weight="bold", color="#00ccff")
        ...
        self.side_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Divider(color="#33363F"),
                ft.Row([ft.Text("💨", size=18), ft.Text(" 本日エアー実績", size=16, weight="bold", color="#00ccff")]),
        ...
        self.controls = [
            ft.Stack([
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Row([ft.Icon(ft.Icons.AIR, color="#00ccff", size=24), ft.Text("エアー清掃 ランキング", size=22, weight="bold"), self.weekly_period_label, ft.Container(expand=True), self.mode_selector], vertical_alignment="center"),
                            ft.Row([
    ...
    def update_tab(self, df_all, date_str):
        ...
        if df_today.empty:
            ...
            if not df_air_only.empty:
                # groupby集計
                model_summary = df_air_only.groupby([ '機種名', 'メーカー' ])[ 'エアー清掃台数' ].sum().sort_values(ascending=False).reset_index()
                    ...
                    for col in self.chart_columns:
                        for ctrl in col.controls:
                            try:
                                ...
                                bar_container.width = 180 * bar_container.data # Webサイズ
                            except: continue
                    self.update()
