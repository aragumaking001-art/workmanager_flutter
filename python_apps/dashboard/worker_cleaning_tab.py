...
class CleaningView(ft.Column):
    def __init__(self):
        ...
        self.char_list = [{"name": "ネコ軍曹", "emojis": ["🐱💤", "🐈🐾", "😸✨", "😼🔥", "😻🏆"], "msgs": ["日向ぼっこ中ニャ", "現場をパトロール中ニャ！", "いいリズムだニャ！", "あと一息！追い込みニャ！", "2000台達成！最高だニャ！"]},{"name": "情熱のライオン", "emojis": ["🦁💤", "🦁🔥", "🦁📢", "🦁💪", "🦁👑"], "msgs": ["王はまだ眠っている…", "エンジンがかかってきたぜ！", "熱い！現場の熱気が伝わるぞ！", "限界を超えろ！ラストスパートだ！", "完全制覇！君こそ真の王（MVP）だ！"]},{"name": "癒やしのパンダ", "emojis": ["🐼💤", "🐼🍃", "🐼✨", "🐼💝", "🐼🎊"], "msgs": ["まだ夢の中だよ…", "笹を食べて準備万端！", "すごいすごい！順調だね〜", "もうすぐ終わるよ、ファイト！", "目標達成！今日はゆっくり休もうね"]},{"name": "爆走ウサギ", "emojis": ["🐰💤", "🐰🥕", "🐇💨", "🐇💥", "🐇🚀"], "msgs": ["耳だけ起きてるよ", "栄養補給（人参）完了！", "風を感じる速さだ！ぴょんぴょん！", "加速装置オン！ゴールは目標は目前！", "2000台突破！光の速さだったね！"]}]
        ...
        self.total_display = ft.Text(value="", size=40, weight="bold", color="#00ffcc")
        ...
        self.today_char = self.char_list[0]
        ...
        self.side_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Divider(color="#33363F"),
                ft.Column([
                    ft.Text(f"🏁 本日目標", size=16, color=ft.Colors.GREY_400, weight="bold"),
                ...
                ft.Divider(color="#33363F"),
                ft.Row([ft.Text("🧼", size=20), ft.Text(" 本日清掃実績", size=18, weight="bold", color="#00ccff")]),
            ...
            ft.Stack([
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Row([
                                ft.Icon(ft.Icons.CLEANING_SERVICES, color="#00ccff"), 
                                ft.Text("通常清掃 ランキング", size=24, weight="bold"), 
    ...
    def update_tab(self, df_all, date_str, df_members=None, df_models=None):
        ...
        try:
            ...
            if self.weekly_period_label.page: 
                self.weekly_period_label.update()
        ...
        if not df_rank.empty:
            ...
            for i in range(3):
                ...
                for idx, (s_id, points) in enumerate(subset.items()):
                    ...
                    name, rank_emoji, bar_color, name_color = raw_name[:10], "", "#00ffcc", "white" 
    ...
    def open_target_dialog(self, e):
        ...
        self.target_dialog = ft.AlertDialog(title=ft.Text("通常清掃 目標台数の変更"), content=ft.Column([ft.Text("本日の通常清掃目標を入力してください。"), self.target_input], height=120, tight=True), actions=[ft.TextButton("キャンセル", on_click=self.close_target_dialog), ft.ElevatedButton("確定", on_click=self.change_target_count, bgcolor="#00ccff", color="black")])
        self.page.overlay.append(self.target_dialog)
        self.target_dialog.open = True; self.page.update()

    def close_target_dialog(self, e): 
        self.target_dialog.open = False; self.page.update()
    ...
    def on_calendar_dismiss(self, e): 
        self.guide_overlay.visible = False; self.page.update()