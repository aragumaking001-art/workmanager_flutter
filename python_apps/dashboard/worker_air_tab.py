...
class AirView(ft.Column):
    def __init__(self):
        ...
        self.target_click_area = ft.Container(content=self.target_display_text, on_click=self.open_target_dialog, ink=True, border_radius=5, padding=ft.padding.symmetric(horizontal=10))
        self.daily_seed = datetime.now().strftime("%Y%m%d")
        random.seed(self.daily_seed)
        self.char_list = [{"name": "ネコ軍曹", "emojis": ["🐱💤", "🐈🐾", "😸✨", "😼🔥", "😻🏆"], "msgs": ["日向ぼっこ中ニャ", "現場をパトロール中ニャ！", "いいリズムだニャ！", "あと一息！追い込みニャ！", "目標達成！最高だニャ！"]},{"name": "情熱のライオン", "emojis": ["🦁💤", "🦁🔥", "🦁📢", "🦁💪", "🦁👑"], "msgs": ["王はまだ眠っている…", "エンジンがかかってきたぜ！", "熱い！現場の熱気が伝わるぞ！", "限界を超えろ！ラストスパートだ！", "完全制覇！君こそ真の王だ！"]},{"name": "癒やしのパンダ", "emojis": ["🐼💤", "🐼🍃", "🐼✨", "🐼💝", "🐼🎊"], "msgs": ["まだ夢の中だよ…", "笹を食べて準備万端！", "すごいすごい！順調だね〜", "もうすぐ終わるよ、ファイト！", "目標達成！今日はゆっくり休もうね"]},{"name": "爆走ウサギ", "emojis": ["🐰💤", "🐰🥕", "🐇💨", "🐇💥", "🐇🚀"], "msgs": ["耳だけ起きてるよ", "栄養補給完了！", "風を感じる速さだ！ぴょんぴょん！", "加速装置オン！ゴールは目前！", "目標突破！光の速さだったね！"]}]
        ...
        self.total_display = ft.Text(value="", size=40, weight="bold", color="#00ccff")
        ...
        self.side_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Divider(color="#33363F"),
                ft.Column([
                    ft.Text(f"🏁 本日エアー目標", size=16, color=ft.Colors.GREY_400, weight="bold"),
                ...
                ft.Divider(color="#33363F"),
                ft.Row([ft.Text("💨", size=20), ft.Text(" 本日エアー実績", size=18, weight="bold", color="#00ccff")]),
        ...
        self.controls = [
            ft.Stack([
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Row([ft.Icon(ft.Icons.AIR, color="#00ccff"), ft.Text("エアー清掃 ランキング", size=24, weight="bold"), self.weekly_period_label, ft.Container(expand=True), self.mode_selector], vertical_alignment="center"),
                            ft.Row([
    ...
    def update_tab(self, df_all, date_str, df_members=None, df_models=None):
        ...
        if 'エアー清掃台数' in df_all.columns:
            df_all['エアー清掃台数'] = pd.to_numeric(df_all['エアー清掃台数'], errors='coerce').fillna(0)
        ...
        try:
            ...
            df_rank = df_rank[df_rank['エアー清掃台数'] > 0].copy()
            
        except Exception as e:
            print(f"AirView Filter Error: {e}"); return
        ...
        if df_today.empty:
            ...
            total_units = int(df_today['エアー清掃台数'].sum())
            ...
            df_air_only = df_today[df_today['エアー清掃台数'] > 0].copy()
            
            if not df_air_only.empty:
                model_summary = df_air_only.groupby(['機種名', 'メーカー'])['エアー清掃台数'].sum().reset_index()
                model_summary = model_summary.sort_values('エアー清掃台数', ascending=False)
                    ...
                    self.model_list.controls.append(
                        ft.Container(
                            content=ft.Row([
                                ...
                                ft.Text(f"{int(row['エアー清掃台数'])}台", size=16, weight="bold", color="#00ccff")
        ...
        if not df_rank.empty:
            def get_pt(row):
                ...
                std_qty = float(info["std_qty"]) if info and "std_qty" in info else 30.0
                unit_pt = 60.0 / std_qty
                base_points = float((int(row.get('エアー清掃台数', 0) or 0)) * unit_pt)
    ...
    def open_target_dialog(self, e):
        ...
        self.target_dialog = ft.AlertDialog(title=ft.Text("エアー目標台数の変更"), content=ft.Column([ft.Text("本日のエアー清掃目標を入力してください。"), self.target_input], height=120, tight=True), actions=[ft.TextButton("キャンセル", on_click=self.close_target_dialog), ft.ElevatedButton("確定", on_click=self.change_target_count, bgcolor="#00ccff", color="black")])
